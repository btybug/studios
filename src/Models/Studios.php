<?php namespace BtyBugPlugins\Studios\Models;
...
class Studios
{
    private static $path = 'vendor'.DS.'sahak.avatar'.DS.'studios'.DS.'src';
    private static $file = 'groups.json';
    public static $viewFolderPath = 'vendor'.DS.'sahak.avatar'.DS.'studios'.DS.'src'.DS.'Resources'.DS.'Views'.DS.'studios';
    public static $studioAssetsPath = 'vendor'.DS.'sahak.avatar'.DS.'studios'.DS.'src'.DS.'Resources'.DS.'Views'.DS.'studio_assets';

    public static function getGroupsList()
    {
        return json_decode(\File::get(base_path(self::$path.DS.self::$file)),true);
    }
    ...
    public static function getStudiosByGroup($group)
    {
        ...
        $data = [];
        if (isset($groups[$group]) && \File::isDirectory(base_path(self::$viewFolderPath . DS . $group))) {
            $files = \File::glob(base_path(self::$viewFolderPath . DS . $group . DS . "*.blade.php"));
            if (count($files)) {
    ...
    {
        if(\File::isDirectory(base_path(self::$studioAssetsPath.DS.'css'))){
            $files =  \File::glob(base_path(self::$studioAssetsPath.DS.'css'.DS.'*.css'));
            if(count($files)){
                foreach ($files as $file){
                     BBstyle($file);
                }
    ...
    {
        if(\File::isDirectory(base_path(self::$viewFolderPath.DS.$type.DS."assets".DS."css"))){
            $files =  \File::glob(base_path(self::$viewFolderPath.DS.$type.DS."assets".DS."css".DS.'*.css'));
            if(count($files)){
                foreach ($files as $file){
                   BBstyle($file);
                }
    ...
    {
        if(\File::isDirectory(base_path(self::$viewFolderPath.DS.$type.DS."assets".DS."js"))){
            $files =  \File::glob(base_path(self::$viewFolderPath.DS.$type.DS."assets".DS."js".DS.'*.js'));
            if(count($files)){
                foreach ($files as $file){
                    echo BBscript($file);
                }
